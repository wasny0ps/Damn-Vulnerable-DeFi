...
Lastly, the `ERC20 Snapshot` feature appears to have an implementation issue that needs to be addressed. The `distributeRewards()` function increments the snapshot ID whenever a new round begins. Consequently, the snapshot ID generated by the `mint()` function in the **previous round will be lower than the snapshot ID for the current round**. This enables **any user who deposits funds at the start of a new round to record their deposit balance for that particular round**.
